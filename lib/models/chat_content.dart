...
class ChatContent {
  ...
  static List<Map<String, dynamic>> _listOfMessageContentsToJson(List<MessageContent> messages) {
  ...
  static ChatContent fromJson(Map<String, dynamic> chatMap) {
    ...
    return ChatContent(
      ...
      imagesPaths: chatMap['imagesPaths'] != null ? List<String>.from(chatMap['imagesPaths']) : null,
  ...
  Future<Image?> getImage(String imageName) async {
    if (imagesPaths == null) {
      return await getImageByPath(imageName);
    }

    /// The file name is like this: "IMG-20220609-WA0012.jpg (file attached)"
    imageName = imageName.split(' ').first;
    String? path = imagesPaths?.firstWhereOrNull((String path) => path.contains(imageName));
    if (path == null) {
      return null;
    }
  ...
  static Future<Image?> getImageByPath(String path) async {
    try {
      ...
      print('Error getting image by path: $e');
      return null;
    }
  }
  ...
  @override
  int get hashCode => hashValues(chatName, sizeOfChat, members, messages, indexesPerMember, msgsPerMember, imagesPaths);
}
